...
def plot_technical_analysis_advanced(df,
    ...
    # Color area between two EMAs
    ax1.fill_between(df['Date'], df[ema_short_col], df[ema_long_col],
                     where=(df[ema_short_col] >= df[ema_long_col]),
                     color='lightgreen', alpha=0.3, label='EMA short > EMA long')
    ax1.fill_between(df['Date'], df[ema_short_col], df[ema_long_col],
                     where=(df[ema_short_col] < df[ema_long_col]),
                     color='lightcoral', alpha=0.3, label='EMA short < EMA long')

    ax1.set_title(f'{title} - Price & EMA', fontsize=14, fontweight='bold')
    ...
    # Color histogram: green when MACD > Signal, red otherwise
    ...
    # Format time axis with specific years
...
def generate_signals(df):
    ...
    # Buy condition:
    # 1. MACD Histogram transitions from negative to positive
    # 2. Price above EMA long (uptrend)
    # 3. EMA short > EMA long (uptrend confirmation)
    buy_condition = (
        (df['MACD_Histogram'] > 0) &
    ...
    # Sell conditions:
    # 1. MACD Histogram transitions from positive to negative
    # 2. Price below EMA long (downtrend)
    # 3. EMA short < EMA long (downtrend confirmation)
    sell_condition = (
        (df['MACD_Histogram'] < 0) &
        (df['MACD_Histogram'].shift(1) >= 0) &
        (df['Close'] < df['EMA_long']) &
...
def optimize_strategy(df):
    ...
    for params in ParameterGrid(param_grid):
        ...
        # Debug: Print trades count for each parameter set
...
if __name__ == "__main__":
    ...
    print(f"Total Return: {best_total_profit:.2%}")
    print(f"Annualized Return: {best_annualized_return:.2%}")
    print(f"Total Trades: {len(best_trades)}")
    print(f"Win Rate: {best_win_rate:.2%}")
    print(f"Average Profit per Trade: {best_avg_profit:.4f}")
    print(f"\nDetailed Trade History:")
    print("=" * 60)
    print(best_trades.to_string())
    print(f"=" * 60 + "\n")
    
    # 8. Plot technical analysis and signals using best_params from optimization
    print("=" * 60)
    print("8. Plotting technical analysis with optimized parameters...")
    print("=" * 60)
    
    # Recalculate data using best parameters
    optimized_data = data.copy()
    optimized_data = calculate_ema(optimized_data, best_params['EMA_short'], best_params['EMA_long'])
    optimized_data = calculate_macd(optimized_data, best_params['MACD_Signal_span'])
    optimized_data = generate_signals(optimized_data)
    
    # Plot technical analysis with optimized parameters
    plot_technical_analysis_advanced(optimized_data, title=f"TSLA Technical Analysis (Optimized: EMA_short={best_params['EMA_short']}, EMA_long={best_params['EMA_long']}, MACD_Signal={best_params['MACD_Signal_span']})")
    print("[OK] Technical analysis chart with optimized parameters plotted\n")
    
    # Plot buy/sell signals with optimized parameters
    print("=" * 60)
    print("9. Plotting buy/sell signals with optimized parameters...")
    print("=" * 60)
    plot_buy_sell_signals(optimized_data)
    print("[OK] Buy/sell signals chart with optimized parameters plotted\n") 